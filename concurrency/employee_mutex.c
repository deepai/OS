...
bool compare_employees(struct employee* e1, struct employee* e2, int i)
{
    struct employee* worker = e1;
        /* compare employees */
    if (e2->id != worker->id) {
    ...
    if (strcmp(e2->first_name, worker->first_name) != 0) {
        printf("mismatching 'first_name' , %s != %s (loop '%d')\n",
        e2->first_name, worker->first_name, i);
        return false;
    }
    if (strcmp(e2->last_name, worker->last_name) != 0) {
        printf("mismatching 'last_name' , %s != %s (loop '%d')\n",
        e2->last_name, worker->last_name, i);
        return false;
    }
    if (strcmp(e2->department, worker->department) != 0) {
        printf("mismatching 'department' , %s != %s (loop '%d')\n",
        e2->department, worker->department, i);
        return false;
    }
    if (e2->room_number != worker->room_number) {
        printf("mismatching 'room_number' , %d != %d (loop '%d')\n",
        e2->room_number, worker->room_number, i);
        return false;
    }
...
int main(int argc, char* argv[])
{
    ...
    for (i=0; i<60000; i++) {
        ...
        if (!compare_employees(&employees[eotd.number-1], &eotd, i)) {
            exit(0);
        }
    }
